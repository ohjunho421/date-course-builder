...
이 미니앱은 **로그인 없이** 코스를 만들고 공유하고 응답받을 수 있습니다. 만든
코스는 이 기기의 localStorage("내 코스")에 보관됩니다. 로그인은 다른 기기에서도
코스를 보관·조회하려는 사용자를 위한 **선택 기능**입니다.

토스 로그인 흐름 (활성화한 경우, 앱인토스 WebView에서는 토스 로그인만 노출):
...
인증서를 넣지 않으면 로그인 옵션 없이 익명으로만 동작합니다(코스 생성·공유·응답
모두 정상). 파트너 API 경로는 공식 문서와 다르면 `src/lib/toss.ts` 상단 상수를
수정하세요 (개발 샌드박스에서 문서 사이트 접근이 차단되어 엔드포인트 경로는 실
인증서로 최종 확인이 필요합니다).
...
2. (선택) 크로스 디바이스 보관을 지원하려면 콘솔에서 토스 로그인 설정 + mTLS
   인증서 발급 → Railway 환경 변수에 등록. 생략하면 익명으로만 동작합니다.
...
- 로그인: 필요 없음 — 익명으로 코스 생성·공유·응답 모두 가능. 로그인 반려
  리스크 없음. 토스 로그인은 크로스 디바이스 보관용 선택 기능.
